...
- **Decision (Tech Debt — auto-recorded) — Phase 2 Review, 2026-05-09:** Suggestion #4 — orientation line at SKILL.md line 16 ("In prose this flag is called `--fast`...") sits above both flag subsections but only mentions `--fast`. Pure polish — could be generalized to mention both flags.
- **Decision (Tech Debt — auto-recorded) — Phase 2 Review, 2026-05-09:** Suggestion #5 — `--auto` branch label in Step 6b (`**--auto branch (precedes paths 1–3)**`) is a bold pseudo-heading, not a markdown heading, so does not appear in TOC. Pure polish — could be promoted to `####` heading if TOC visibility matters.
