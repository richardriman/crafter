...
### Decision (Implementer Recorded — Phase 2 Step 2): ID generation scheme — 2026-05-09

**Chosen:** 12-character lowercase hex string from `crypto/rand` (6 random bytes, hex-encoded). Generated by the Go subcommand at append time; not a caller flag.

**Rationale:** No new dependencies — `crypto/rand` is in the standard library. 6 bytes = 2^48 ≈ 281 trillion unique values, more than sufficient for a per-run append-only log. Consistent with the existing `skillbook.NewID` pattern (which uses 8 bytes / 16 chars; we use 6 bytes / 12 chars to keep entries shorter while staying well within the 512-byte cap). ULID was considered but rejected — it would add an external dependency and the time-ordering it provides is already covered by `created_at`.

### Decision (Implementer Recorded — Phase 2 Step 2): Entry size cap — 2026-05-09

**Chosen:** 512 bytes per entry (including the trailing LF). Constant `MaxEntryBytes = 512` defined in `cli/internal/buffer/store.go`. Entries exceeding this limit are rejected with a clear error before the file is opened (no partial write, no file creation).

**Rationale:** D3 deferred this to Phase 2. POSIX guarantees write(2) to an O_APPEND file is atomic up to PIPE_BUF. The conservative bound is 512 bytes on macOS (per POSIX minimum). Linux is 4096 bytes but we target the smallest common denominator so the guarantee holds on both platforms. Entries that exceed 512 bytes are rejected with a clear error message that names the buffer file path and mentions the cap; the caller is instructed to shorten the relevant flag value.

### Decision (Implementer Recorded — Phase 2 Step 2): `created_by` and `task_id` as caller-provided flags — 2026-05-09

**Chosen:** `--created-by` and `--task-id` are required flags passed by the caller (not generated by the subcommand). The Go subcommand generates only `id`, `kind`, and `created_at`.

**Context:** SKILL.md (Phase 2 Step 1) listed `created_by` and `task_id` in the "subcommand generates" sentence alongside `id`, `kind`, `created_at`. This was identified as a Phase 2 Step 1 oversight per the task prompt's explicit instruction to "add `--created-by` and `--task-id` as REQUIRED flags." The caller (orchestrator / calling skill) already knows the active task ID and the calling agent identity; having the subcommand look them up from the environment would add fragile implicit state. Explicit required flags are simpler, testable, and consistent with `--run-dir` (which the caller also provides).

**SKILL.md impact:** SKILL.md's "Entry shape" section prose ("The subcommand generates `id`, `kind`, `created_at`, `created_by`, and `task_id`") is technically incorrect for `created_by` and `task_id`. The SKILL.md is the contract for callers; this implementation treats the task prompt as the authoritative override. A SKILL.md correction is out of scope for this step (SKILL.md is Phase 2 Step 1's artifact, and the scope boundary says do NOT modify SKILL.md in this step). The discrepancy is noted here for Phase 3 / review to address.

### Decision (Implementer Recorded — Phase 2 Step 2): Missing run-dir behavior — 2026-05-09

**Chosen:** Error out with a clear OS-level error ("opening buffer file … no such file or directory"). Do NOT auto-create the directory. Auto-creating is a Phase 3 lifecycle concern.

**Rationale:** The `--run-dir` directory is created by the orchestrator before the first buffer call (documented in SKILL.md "Creation behavior" section: "The `--run-dir` directory must exist before `crafter buffer` is called"). Silently creating missing directories would hide caller errors and conflict with Phase 3's lifecycle design.

### Decision (Implementer Accepted — Phase 2 Step 3): SKILL.md aligned with Step 2 implementation decisions — 2026-05-09

**Summary:** `skills/crafter-buffer/SKILL.md` was corrected to match the four Phase 2 Step 2 implementation decisions:

1. The "Entry shape" prose now states that the subcommand generates only `id`, `kind`, and `created_at`; `created_by` and `task_id` are caller-provided required flags.
2. Both Operations sections (`crafter buffer uat` and `crafter buffer gap`) now list `--created-by` and `--task-id` as required flags between `--source` and the kind-specific flags, in both the shell invocation snippets and the "Required flags" enumeration lines.
3. All three CLI invocation snippets in the Escaping reference section now include `--created-by` and `--task-id`, making them runnable as written.
4. A note was added in the "Entry shape" section explaining the convention: pass the calling agent or skill name as `--created-by` (e.g. `crafter-implementer`, `crafter-reviewer`, `crafter-do`) and the task-file basename without extension as `--task-id`.

**Installer finding:** `install.sh` enumerates skills explicitly (not recursively) — `crafter-buffer` was not in the list. One line was added to `install.sh` to `mkdir -p` and `cp` `skills/crafter-buffer/SKILL.md` alongside the other four skills. This is a real one-line edit justified by the explicit enumeration pattern in the installer; no scope change required.

**CLI build pipeline:** `cli/Makefile` uses `go build .` with no skill- or command-specific enumeration. The new `crafter buffer` cobra subcommand is linked automatically via `init()` registration — no Makefile change needed. Confirmed: `go build ./...` and `go test ./...` both pass; `crafter buffer uat --help` and `crafter buffer gap --help` show all required flags including `--created-by` and `--task-id`.

### Decision (Orchestrator Accepted — Phase 3 Step 1): Run-directory creation strategy — 2026-05-10

**Chosen:** Eager creation at Step 4 (Execute) start — the orchestrator runs `mkdir -p .crafter/run/<task-id>/` before spawning the first agent for the first plan step.

**Rationale:** `crafter buffer` requires the `--run-dir` directory to exist and does not create it (Decision 7 / Phase 2 Step 2). Lazy creation (deferring to the first buffer call) is therefore impossible without changing that policy. Eager creation at Step 4 — rather than at Step 0 (Resume Detection) or Step 3 (Plan) — avoids creating empty directories for runs abandoned before execution starts. Resume semantics are handled naturally: `mkdir -p` on an already-existing directory is a no-op, so buffer files from a prior session in the same task are preserved and new entries are appended to them.

**`<task-id>` resolution confirmed:** `<task-id>` is the task-file basename without extension (e.g., `20260509-feat-gh-16-buffer-skill`). This is consistent across `skills/crafter-buffer/SKILL.md` ("task-file basename without extension"), the `### Assumptions` in this task plan, and the `--task-id` flag convention in the Go subcommand. No ambiguity found; assumption locked in as documented.

### Decision (Orchestrator Accepted — Phase 3 Step 1): No per-run metadata artifact — 2026-05-10

**Chosen:** No `meta.json` or equivalent run-marker file inside `.crafter/run/<task-id>/`. The run directory contains only the two buffer files (`uat-buffer.jsonl`, `gaps-buffer.jsonl`) as entries are appended.

**Rationale:** Each buffer entry already carries `task_id` and `created_at` fields (Decision 1), providing sufficient per-entry traceability for the current PoC. Adding a run-level metadata file would expand surface beyond what GH#16 requires, introduce an additional creation/cleanup concern, and pre-commit design choices (schema, creation timing) that belong to GH#17 (PR composer) once there is a concrete consumer for run-level metadata. If GH#17 or GH#18 finds a concrete need (e.g., run-start timestamp, source branch, originating skill/flag), a metadata artifact can be added then.

### Decision (Tech Debt — auto-recorded under `--fast`, Phase 3 Review): 2026-05-10
