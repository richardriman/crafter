...
`crafter-do` core-capability decomposition Slice 6 complete on branch `refactor/crafter-do-slice-6-steps-6-6b-6a` (Steps 6/6b/6a Review+PhaseSummary+SessionBreak extracted — the highest-density slice; both phases delivered, verified, reviewed). Branch ready for PR. Slices 1–5 already on `main`. Only Slice 7 (Steps 7–9/9b) remains to finish the maintenance decomposition; then the real goal: progressive on-demand workflow loading (see `## Planned`).
