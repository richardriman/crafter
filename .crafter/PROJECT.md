...
| 2026-07-03 | Skill-directive injection enforced at the SKILL.md spawn sites (a `## Pre-Spawn Gate` above the first spawn + a co-located level marker at all 14 Task-spawn / re-delegation paths), not only by the distant `rules/delegation.md` rule | The propagation rule was a soft distant reminder the orchestrator skipped at spawn time (0/13 subagents got the directive in a real run); co-locating enforcement where every spawn physically routes through removes the adherence gap without duplicating the directive block or changing the audience policy |
