...
**Progressive-loading task — Phase 0+1** (task `.crafter/tasks/20260518-crafter-do-progressive-loading.md`) absorbed the repo-wide `~/.claude/...` → `{CRAFTER_HOME}` normalization mandate from `.crafter/tasks/20260421-skills-first-runtime-portability.md`, added the centralized install-time `sed` substitution to `install.sh`'s `install_to()`, and normalized all normalize-classified source files (`skills/crafter-debug/SKILL.md` ×9, `skills/crafter-map-project/SKILL.md` ×2, `rules/post-change.md` ×4, `rules/delegation.md` ×2). Repo-wide normalization is COMPLETE; only the multi-runtime build/transform pipeline + runtime adapters (VS Code / Copilot CLI / OpenCode) remain with `.crafter/tasks/20260421-skills-first-runtime-portability.md`.

**Progressive-loading task — Phase 2+3** (task `.crafter/tasks/20260518-crafter-do-progressive-loading.md`, commit `db70fd0`) redesigned `skills/crafter-do/SKILL.md` as a master-plan hybrid-delegating orchestrator: eager spine (4 core rules upfront, no per-step module loading), inline Workflow Master Plan navigation map (full step sequence, scope/flag branching, three resume entry points) enabling full workflow routing without any step module in context, and hybrid per-step delegation via existing role-agents plus new `agents/crafter-step-runner.md` for glue steps. Phase 3 semantic-equivalence verification confirmed 13/13 binding parity and 64/0 install tests. The progressive-loading end goal is complete on branch `feat/crafter-do-progressive-loading`.
